...
public class AlertManager : MonoBehaviour
{
    ...
    public void Alert(string message)
    {
        ...
        alert.transform.GetChild(0).GetComponent<TextMeshProUGUI>().SetText(message);
    ...
    IEnumerator WaitToShow(GameObject alert)
    {
        ...
        LeanTween.scale(alert, Vector2.one, 1f).setEase(LeanTweenType.easeInOutBack);
    }
    ...
    IEnumerator DestroyAlert(GameObject alert)
    {
        ...
        LeanTween.scale(alert, Vector2.zero, 1f).setEase(LeanTweenType.easeInBack).setOnComplete(() => Destroy(alert));
    }
}
